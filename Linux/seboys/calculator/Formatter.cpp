...
Formatter::Formatter(const cv::Rect &cropBox,
...
int Formatter::crop(const cv::Mat &inputImage, cv::Mat &outputImage) const {
...
int Formatter::warp(const std::vector<LineInfo> originalLines,
                    std::vector<LineInfo> &outputLines) const {
    outputLines.clear();
    ...
    for (int i = 0; i < N_LINES; i++) {
        outputLines.push_back(LineInfo(Line(startingPoints[i], endingPoints[i])));
    }
