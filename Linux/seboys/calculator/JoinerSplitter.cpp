...
void JoinerSplitter::splitImageToSections(const cv::Mat &image, std::vector<Section> &sections) const {
    ...
    for (float ratio : _ratios) {
        ...
        sections.push_back(Section(sectionIndex, sectionArea, image));
...
void JoinerSplitter::joinSectionsToImage(const std::vector<Section> &sections, cv::Mat &image) {
